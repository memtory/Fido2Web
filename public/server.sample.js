...
/* Password section */
    let registerPassword = (payload) => {
        ...
        if(db.userExists(payload.username) && db.getUser(payload.username).registrationComplete)
            return Promise.reject({'status': 'failed', 'errorMessage': 'User already registered!'})
...
/* MakeCred sections */
    ...
    let makeCredentialResponse = (payload) => {
        ...
        var clientdata = JSON.parse(enc.decode(base64url.decode(payload.response.clientDataJSON)))
        return Promise.resolve({'status': 'ok','id': payload.id, 'clientDataJson': clientdata, 'attestationObject' : payload.response.attestationObject})
    }
...
/* GetAssertion section */
    ...
    let getAssertionResponse = (payload) => {
        ...
        var enc = new TextDecoder();
        var clientdata = JSON.parse(enc.decode(base64url.decode(payload.response.clientDataJSON)))
        return Promise.resolve({'status': 'ok','id': payload.id, 'clientDataJson': clientdata, 'authenticatorData' : payload.response.authenticatorData, 
        'userHandle': payload.response.userHandle, 'signature': payload.response.signature})
    }
/* GetAssertion ends */
